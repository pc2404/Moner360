...
function x(o, e) {
  ...
  if (
    !o.find((n) => {
      ...
      let t;
      return (
        typeof n == "string" ? (t = new RegExp(`^${n}`)) : (t = n),
...
function q(o, e, s) {
  ...
  const n = O(o.access_token),
    t = { ...o, accessTokenPayload: n };
  e.hideAccessToken && (t.access_token = _.ACCESS_TOKEN + "_" + s),
    (o.accessTokenPayload = n);
  let i = null;
  if (o.id_token) {
    if (
      ((i = O(o.id_token)),
  ...
  const d = i && i.exp ? i.exp : Number.MAX_VALUE,
    l = n && n.exp ? n.exp : o.issued_at + o.expires_in;
  let f;
  ...
  const c = e.nonce ? e.nonce.nonce : null,
    { isValid: h, reason: r } = b(o, c, e.oidcServerConfiguration);
...
const D = (o) => new Promise((e) => setTimeout(e, o)),
  p = self;
p.importScripts(N);
const V = Math.round(new Date().getTime() / 1e3).toString(),
  L = "OidcKeepAliveServiceWorker.json",
  H = (o) => {
    console.log("[OidcServiceWorker] service worker installed " + V),
      o.waitUntil(p.skipWaiting());
  },
  j = (o) => {
    console.log("[OidcServiceWorker] service worker activated " + V),
      o.waitUntil(p.clients.claim());
  };
let m = null;
const E = {
    default: {
  ...
  J = (o, e) => {
    ...
    for (const [, n] of Object.entries(o))
      ((n.oidcServerConfiguration != null &&
        e.startsWith(n.oidcServerConfiguration.tokenEndpoint)) ||
        (n.oidcServerConfiguration != null &&
          n.oidcServerConfiguration.revocationEndpoint &&
          e.startsWith(n.oidcServerConfiguration.revocationEndpoint))) &&
        s.push(n);
    return s;
  },
  Y = async (o) => {
    const e = o.request,
      ...
      n = { status: 200, statusText: "oidc-service-worker" },
      t = new Response("{}", n);
    if (!s) {
      const i = new URL(e.url),
        d = Number(i.searchParams.get("minSleepSeconds")) || 240;
      for (let l = 0; l < d; l++)
        await D(1e3 + Math.floor(Math.random() * 1e3)),
          await (
            await caches.open("oidc_dummy_cache")
          ).put(o.request, t.clone());
    }
    return t;
  },
  z = async (o) => {
  ...
  $ = (o) => {
    ...
    if ((trustedDomains == null && (trustedDomains = {}), !t)) {
      ...
      (E[n] = {
        ...
        console.log(E[n]),
        (t = E[n]),
        trustedDomains[n] || (trustedDomains[n] = []);
    }
    switch (s.type) {
      case "clear":
        (t.tokens = null),
          ...
          e.postMessage({ configurationName: n });
        return;
      case "init": {
        ...
        if (
          ...
          const a = { ...t.tokens };
          t.hideAccessToken && (a.access_token = _.ACCESS_TOKEN + "_" + n),
            a.refresh_token && (a.refresh_token = _.REFRESH_TOKEN + "_" + n),
            a.idTokenPayload &&
              a.idTokenPayload.nonce &&
              t.nonce != null &&
              (a.idTokenPayload.nonce = _.NONCE_TOKEN + "_" + n),
            e.postMessage({
              tokens: a,
              status: t.status,
              configurationName: n,
      ...
      case "getCodeVerifier": {
        e.postMessage({
          configurationName: n,
          codeVerifier: _.CODE_VERIFIER + "_" + n,
        });
        return;
      }
